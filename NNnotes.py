# IMPORTS
from numpy import random, exp, exp2, tanh, heaviside, sin, array#, dot, transpose
...
def mutateBitstring(bitstring:"str", bonus):
    return bitCombine(bitstring, generateMutationBitstring(36, bonus))### 36???
...
def funcSine(generation):# f(x)=mx+A\sin(Bx) m=0.7, A=8, B=0.35
    m = 0.7
    A = 8
    B = 0.35
...
class NeuralNetwork():
    ...
    #
    # NeuralNetwork wants to take a Genome which has /already/ been blasted with rads and proceed to submerge it in a toxic waste bath before building it.
    def __init__(self, outputCount:'int'=1, parentGenome:'list[str]'=None, genomeToxicWasteBathPercentage:"float"=0.01, generation:"int"=1):
        ...
        genomeCurrent = list()# for every gene, make it a bitstring, mutate it, then turn it back into hexString and store it in genome.
        for i in range(len(parentGenome)):# run through all the genes in the genome# Decode all the connections into tuples.# for synapse(index) in the_genome:
            ...
            genomeCurrent.append(binToHex(bitstring))## (Note to self: Calm down, that's why this is on it's own line..)
            decodedSynapse = (# disect it
                int(bitstring[0]),# 0 is inputInput, 1 is internal input. 1 bit                 # input type
                int(bitstring[1:8], 2),# Specify which node by its ID in that group. 7 bits     # input source
                int(bitstring[8]),# 0 is output, 1 is internal. 1 bit                           # output type
                int(bitstring[9:16], 2),# 7 bits                                                # output destination
                int(bitstring[16:32], 2) / 8000,# 16 bits                                       # weight value
                int(bitstring[32:], 2)# Bias as int representing an index in the list of preset biases. # 4 bits to work with gives me a usable range of 0 to 15, which I map to -8 to +7.
            ...
            if decodedSynapse[2]:# if the output( from this gene) is an internal node, create that node.
                if not any(decodedSynapse[3] == id for id in self.internalNeurons.keys()):# Check if that Node ID already exists.
    ...
    def proccessInternalNodeLayer(self, workingLayer:'list[int]'):# Layer here is a list of IDs referencing specific Neurons in self.internalNeurons which must be proccessed.
        ...
        ### for every Node in the provided layer of IDs, set the output state of that Node while also resetting it's input cache.
    ...
    def think(self, inputVector:'list[float]'):# forward pass # The neural network thinks.
        ...
        for connLayerIndex in range(len(self.layersSynapse)):# go back and forth between running a connection layer and a node layer. For each Cycle layer...
            for synapse in self.layersSynapse[connLayerIndex]:# find out what you are mutiplying by this weight: Working Input.
                ## what if chance finds a way to try to store into a nonexistant node? how? 
                if synapse[0]:# if input's coming from an internal neuron
                    result = synapse[4] * self.internalNeurons[synapse[1]]# get data from which Node.
                else:# if input's coming from a raw input
                    try:
                        result = synapse[4] * inputVector[synapse[1]]# get data from which input.
            ...
            # until the last node layer which needs to be skipped( because they're the output nodes).
